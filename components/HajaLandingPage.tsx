...
const CTAButton = ({ href, children, variant = 'dark' }: { href: string; children: React.ReactNode; variant?: 'dark' | 'gold' | 'light' }) => {
  const className =
    variant === 'gold'
...
const CheckItem = ({ children }: { children: React.ReactNode }) => (
  <li className="flex gap-4 rounded-2xl bg-white px-5 py-4 shadow-sm ring-1 ring-slate-200">
    ...
    <span className="text-base md:text-lg font-semibold leading-relaxed text-slate-700">{children}</span>
  </li>
);

const InfoCard = ({ number, title, children, tone = 'white' }: { number?: string; title: string; children: React.ReactNode; tone?: 'white' | 'beige' | 'navy' }) => {
  const toneClass =
    tone === 'navy'
  ...
  return (
    <div className={`rounded-[2rem] p-6 md:p-8 shadow-sm ring-1 ${toneClass}`}>
      ...
      <div className={`mt-4 text-sm md:text-base leading-relaxed ${tone === 'navy' ? 'text-white/80' : 'text-slate-600'}`}>{children}</div>
    </div>
  );
};
...
const ProcessStep = ({ no, title, desc }: { no: string; title: string; desc?: string }) => (
  <div className="relative rounded-3xl border border-[#B89445]/45 bg-white p-5 shadow-sm">
    ...
    <h4 className="text-xl font-black leading-snug text-slate-900 whitespace-pre-line">{title}</h4>
  ...
  return (
    <main className="min-h-screen bg-[#f6f1e8] text-slate-900">
      ...
      <div className="fixed bottom-4 left-1/2 z-50 flex w-[calc(100%-24px)] max-w-[620px] -translate-x-1/2 gap-2 rounded-3xl bg-[#08213D]/95 p-2 shadow-2xl backdrop-blur md:hidden">
        <a href="tel:1688-5644" className="flex-1 rounded-2xl bg-white px-4 py-3 text-center text-sm font-black text-[#08213D]">
          전화 상담
        </a>
        <a href="/#consultation-form" className="flex-1 rounded-2xl bg-[#B89445] px-4 py-3 text-center text-sm font-black text-white">
          상담 신청
        </a>
      </div>
      ...
      <section className="relative overflow-hidden bg-[#08213D] text-white">
        ...
        <div className="relative mx-auto grid min-h-[92vh] max-w-7xl items-center gap-10 px-6 py-20 md:grid-cols-[1.05fr_0.95fr] md:px-10">
          <div>
            <p className="mb-6 text-sm font-black tracking-[0.28em] text-[#D8C084]">APARTMENT DEFECT LITIGATION</p>
          ...
          <div className="rounded-[3rem] border border-white/10 bg-white/10 p-8 shadow-2xl backdrop-blur">
            <div className="mb-10 flex items-center gap-4">
              <div className="flex h-16 w-16 items-center justify-center rounded-full border border-[#D8C084] text-3xl font-black text-[#D8C084]">安</div>
              <div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-24 md:px-10">
        ...
        <SectionLead>분쟁을 단순한 소송으로 보지 않고, 자산가치 회복과 실질적 해결을 함께 설계합니다.</SectionLead>
        ...
        <div className="mt-12 grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-[2.5rem] bg-white p-8 shadow-sm ring-1 ring-slate-200 md:p-10">
            <div className="space-y-7 text-lg leading-relaxed text-slate-700">
              <p>안재현 변호사 법률사무소는 의뢰인의 분쟁을 단순한 소송 사건으로만 보지 않습니다. 사건의 배경, 분쟁의 구조, 향후 회수 가능성까지 함께 검토하여 실질적인 해결 방향을 제시하는 것을 목표로 합니다.</p>
              <p>하자소송은 법률 검토만으로 진행되는 사건이 아닙니다. 건축자료 분석, 하자진단, 감정절차, 입주민 동의, 입주자대표회의 의사결정이 함께 움직이는 복합적인 분쟁입니다.</p>
              <p>대표변호사의 직접 검토 아래 송무 진행, 자료 정리, 감정 대응, 입주민 안내까지 사건 진행 전 과정을 체계적으로 관리합니다.</p>
            </div>
          </div>
      ...
      <section className="bg-white py-24">
        <div className="mx-auto max-w-7xl px-6 md:px-10">
          ...
          <SectionLead>사법시험 출신 경력과 기업 법무 경험을 바탕으로 하자소송의 법률·실무 구조를 함께 검토합니다.</SectionLead>

          <div className="mt-12 grid gap-8 lg:grid-cols-[0.95fr_1.05fr]">
            <div className="rounded-[2.5rem] bg-[#f6f1e8] p-8 shadow-sm ring-1 ring-slate-200">
              <div className="mb-8 flex items-center gap-5">
                <div className="flex h-16 w-16 items-center justify-center rounded-full border border-[#B89445] text-3xl font-black text-[#B89445]">安</div>
                <div>
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={`${label}-${value}`} className="flex items-center gap-4 border-b border-slate-200 pb-3">
                    <span className="w-16 rounded-lg bg-white px-3 py-2 text-center text-sm font-black text-[#B89445]">{label}</span>
            ...
            <div className="flex flex-col justify-between rounded-[2.5rem] bg-[#08213D] p-8 text-white shadow-xl md:p-10">
              <div>
                ...
                <p className="mt-6 text-lg leading-relaxed text-white/80">하자소송은 법률 검토, 건축자료 분석, 하자 적출 및 감정, 입주민 설명이 결합된 복합 분쟁입니다. 감정 절차와 회수 구조까지 고려한 전략 수립이 필요합니다.</p>
              </div>
              <div className="mt-10 rounded-3xl bg-white/10 p-6 text-xl font-black leading-relaxed">
                단순히 소장을 제출하는 데 그치지 않고,<br />단지별 회수 가능성을 기준으로 전략을 설계합니다.
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-24 md:px-10">
        ...
        <SectionLead>주요 하자소송 및 건축 분야 사건 수행 경험을 바탕으로 단지별 하자 구조와 소송 절차를 검토합니다.</SectionLead>

        <div className="mt-12 grid gap-8 lg:grid-cols-2">
          <InfoCard title="하자소송 수행 단지">
            <ul className="grid gap-3 text-lg font-semibold text-slate-700">
              {['동탄역○○지오 하자소송', '원주○○더원그레이스 하자소송', 'e편한세상 독○○타워 하자소송', '트○○제주더힐 하자소송', '범어○○포레스트 하자소송', '창원메○○시티2단지 하자소송', '증평○○루휴티스 하자소송', '부산센○○르지오 하자소송', '㈜더쿨○○트호텔 하자소송'].map((item) => (
                <li key={item} className="flex gap-3"><span className="text-[#B89445]">•</span>{item}</li>
              ))}
            </ul>
          </InfoCard>
          <InfoCard title="공사대금 및 건축분쟁" tone="beige">
            <ul className="grid gap-3 text-lg font-semibold text-slate-700">
              {['㈜극동에이앤씨 공사대금', '○○삼환아파트 공사대금', '㈜라라건설 공사대금', '㈜수명엔지니어링 공사대금'].map((item) => (
                <li key={item} className="flex gap-3"><span className="text-[#B89445]">•</span>{item}</li>
              ))}
            </ul>
          </InfoCard>
      ...
      <section className="bg-white py-24">
        <div className="mx-auto max-w-7xl px-6 md:px-10">
          ...
          <SectionLead>하자소송은 법률 검토만으로 끝나는 사건이 아닙니다. 건축자료, 하자적출, 채권양도, 입주민 설명이 함께 움직여야 합니다.</SectionLead>

          <div className="mt-12 grid gap-6 md:grid-cols-3">
            <InfoCard number="01" title="건축·하자소송 수행 경험">아파트·집합건물 하자소송, 공사대금 분쟁 수행 경험을 바탕으로 검토합니다.</InfoCard>
            <InfoCard number="02" title="입대의 의사결정 지원" tone="beige">회의자료, 입주민설명회, 채권양도 절차를 함께 고려합니다.</InfoCard>
            <InfoCard number="03" title="감정 절차 중심 대응">현장조사, 감정보고서, 보완감정, 조정 전략을 관리합니다.</InfoCard>
          </div>
          ...
          <div className="mt-10 rounded-3xl border border-[#B89445]/40 bg-[#F2EBDD] p-8">
            ...
            <p className="mt-4 text-lg leading-relaxed text-slate-700">법무팀장·법무이사 경력을 바탕으로 한 실무형 대응. 하자소송은 “소송”이면서 동시에 단지의 “자산관리” 전략입니다.</p>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-6 py-24 md:px-10">
        ...
        <SectionLead>반복되는 누수·균열·결로·타일 들뜸은 단순 민원이 아니라 단지의 손해일 수 있습니다.</SectionLead>
      ...
      <section className="bg-white py-24">
        <div className="mx-auto max-w-7xl px-6 md:px-10">
          ...
          <SectionLead>입주자대표회의가 하자소송을 검토할 때 가장 먼저 부딪히는 문제는 비용, 동의, 기간, 책임 부담입니다.</SectionLead>
          ...
          <div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {[
              ...
              <InfoCard key={no} number={no} title={title} tone={idx % 2 ? 'beige' : 'white'}>{desc}</InfoCard>
            ))}
          </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-24 md:px-10">
        ...
        <SectionLead>초기비용 부담을 줄이고, 단지별 하자 규모와 법적 회수 가능성을 기준으로 하자소송 전략을 설계합니다.</SectionLead>
        ...
        <div className="mt-12 grid gap-8 lg:grid-cols-[0.85fr_1.15fr]">
          <div className="rounded-[2.5rem] bg-[#08213D] p-8 text-white shadow-xl md:p-10">
            ...
            <h3 className="mt-4 text-4xl font-black leading-tight">입주민 초기부담<br />0원 구조</h3>
            <p className="mt-6 text-sm leading-relaxed text-white/70">구체적인 비용 부담 범위와 정산 방식은 단지별 검토 후 약정 기준에 따라 정리됩니다.</p>
          </div>
      ...
      <section className="bg-white py-24">
        <div className="mx-auto max-w-7xl px-6 md:px-10">
          ...
          <SectionLead>공동주택 하자는 생활 불편을 넘어 보수비용, 안전 문제, 자산가치 하락으로 이어질 수 있습니다.</SectionLead>
          ...
          <div className="mt-8 grid gap-5 md:grid-cols-2">
            <InfoCard title="공용부 하자">지하주차장, 옥상, 외벽, 계단, 승강기, 설비 배관</InfoCard>
            <InfoCard title="전유부 하자" tone="beige">세대 내부 누수, 결로, 마감 불량, 창호, 타일, 욕실</InfoCard>
            <InfoCard title="시공상 하자">미시공, 오시공, 설계와 다른 시공, 저품질 자재 사용</InfoCard>
            <InfoCard title="기능·미관·안전 하자" tone="beige">균열, 누수, 곰팡이, 탈락, 침하, 소음, 설비 고장</InfoCard>
          </div>

          <div className="mt-10 rounded-3xl border border-[#B89445]/40 bg-[#F2EBDD] p-8">
            <h3 className="text-2xl font-black text-[#08213D]">하자는 민원이 아니라, 회수해야 할 손해입니다.</h3>
            <p className="mt-4 text-lg leading-relaxed text-slate-700">하자소송은 단순 보수 요청이 아니라, 입주민이 부담해서는 안 될 손해를 법적으로 회수하는 절차입니다.</p>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-6 py-24 md:px-10">
        ...
        <SectionLead>하자소송은 하나의 절차처럼 보이지만, 실제 청구 구조는 하자 발생 시점과 성격에 따라 나뉩니다.</SectionLead>
          ...
          <div className="overflow-hidden rounded-[2.5rem] bg-white shadow-sm ring-1 ring-slate-200">
            ...
            <div className="divide-y divide-slate-200 p-8">
              {[
                ...
                <p key={label} className="grid grid-cols-[70px_1fr] gap-4 py-5 text-lg"><span className="font-black text-[#B89445]">{label}</span><span className="font-bold text-slate-700">{text}</span></p>
              ))}
            </div>
          </div>
          <div className="overflow-hidden rounded-[2.5rem] bg-white shadow-sm ring-1 ring-slate-200">
      ...
      <section className="bg-white py-24">
        <div className="mx-auto max-w-7xl px-6 md:px-10">
          <SectionLabel>APARTMENT DEFECT LITIGATION</SectionLabel>
          <SectionTitle>우리 법은 입주민의 편이지만,<br />시간은 시공사의 편입니다</SectionTitle>
          <SectionLead>집합건물법상 하자담보책임의 제척기간이 도과하기 전, 사용검사일과 하자보수 이력을 먼저 확인해야 합니다.</SectionLead>
          ...
          <div className="mt-12 rounded-[2.5rem] bg-[#f6f1e8] p-6 shadow-sm ring-1 ring-slate-200 md:p-10">
            ...
            <p className="mt-3 text-slate-600">기간이 지나면 청구 가능한 범위가 줄어들 수 있으므로, 초기 검토가 중요합니다.</p>
            <div className="mt-8 grid gap-5 md:grid-cols-3">
              <InfoCard title="2~3년 하자" tone="white"><strong className="text-[#08213D]">마감·설비</strong><br />마감공사·건축설비, 도배·타일, 목공사, 창호, 조경공사</InfoCard>
              <InfoCard title="5년 하자" tone="white"><strong className="text-[#08213D]">구조·방수</strong><br />미시공·오시공·방수, 대지조성, 철근콘크리트, 방수공사</InfoCard>
              <InfoCard title="10년 하자" tone="white"><strong className="text-[#08213D]">주요 구조부</strong><br />건물의 주요 구조부 및 지반공사, 치명적 하자는 장기 검토 대상</InfoCard>
            </div>
          ...
          <div className="mt-10 rounded-3xl bg-[#08213D] px-8 py-6 text-xl font-black leading-relaxed text-white">
            기간이 지나면 청구가 어려워질 수 있습니다.<br />2013년 6월 이후 분양 아파트는 제척기간 검토가 특히 중요합니다. 사용검사일과 하자보수 이력부터 먼저 확인해야 합니다.
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-6 py-24 md:px-10">
        ...
        <SectionLead>하자진단 및 적출부터 채권양도, 소장 접수, 감정, 판결, 소송비용 상환까지 단계별로 진행됩니다.</SectionLead>
      ...
      <section className="bg-white py-24">
        <div className="mx-auto max-w-7xl px-6 md:px-10">
          ...
          <SectionLead>초기 검토에서는 사용검사일, 하자 발생 경위, 보수 이력, 입주민 민원자료를 기준으로 청구 가능성을 먼저 확인합니다.</SectionLead>

          <div className="mt-12 rounded-[2.5rem] bg-[#f6f1e8] p-6 shadow-sm ring-1 ring-slate-200 md:p-10">
            <h3 className="mb-8 text-2xl font-black text-[#08213D]">준비자료 체크리스트</h3>
            <ul className="grid gap-4 md:grid-cols-2">
              {['사용검사일 확인 자료', '하자보수 요청 내역', '시공사·시행사 답변 자료', '하자보수 예치금 사용 내역', '입주민 민원 자료', '공용부 하자 사진', '관리사무소 보수 이력', '기존 하자진단 보고서', '입주자대표회의 회의록'].map((item) => (
      ...
      <section className="mx-auto max-w-7xl px-6 py-24 md:px-10">
        ...
        <SectionLead>자주 확인하는 질문을 기준으로 하자소송 검토 포인트를 정리했습니다.</SectionLead>

        <div className="mt-12 grid gap-5">
          <FAQItem q="Q1. 하자예치금을 이미 사용했는데도 소송이 가능한가요?" a="사용검사 후 하자보수보증금 청구와 사용검사 전 하자 손해배상청구는 별도로 검토될 수 있습니다." />
          <FAQItem q="Q2. 입주민 전원이 동의해야 하나요?" a="채권양도율은 회수 규모와 절차 안정성에 영향을 주므로, 설명자료와 동의 절차를 체계적으로 준비해야 합니다." />
          <FAQItem q="Q3. 패소하면 비용 부담이 생기나요?" a="비용 부담 구조는 약정 기준으로 정리되며, 사전 검토 단계에서 범위와 정산 방식을 명확히 확인해야 합니다." />
          <FAQItem q="Q4. 소송기간은 어느 정도 걸리나요?" a="법원 감정, 보완감정, 조정 여부에 따라 달라지므로 단지별 하자 규모와 사건 진행 상황을 기준으로 안내합니다." />
        </div>
      </section>
      ...
      <section className="bg-white py-24">
        <div className="mx-auto max-w-7xl px-6 md:px-10">
          <SectionLabel>APARTMENT DEFECT LITIGATION</SectionLabel>
          <SectionTitle>전문성 및 공적 활동 현황</SectionTitle>
          <SectionLead>등록증서, 위촉장, 협약서로 확인하는 신뢰 기반</SectionLead>

          <div className="mt-12 grid gap-5 md:grid-cols-3">
            <InfoCard title="공식 자격">등록 전문분야 및 사법시험 합격 이력</InfoCard>
            <InfoCard title="공공기관 위촉" tone="beige">경찰서 수사민원 법률자문 상담</InfoCard>
            <InfoCard title="대외 협력">업무협약 및 자문 기반</InfoCard>
          </div>
          <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {['사법시험 합격증명서', '사법연수원 수료증', '이혼전문 등록증서', '형사전문 등록증서', '인천서부경찰서 위촉장', '인천계양경찰서 위촉장', '민주노총 업무제휴협약서'].map((item) => (
              <div key={item} className="rounded-3xl bg-[#f6f1e8] p-6 text-center font-black text-[#08213D] ring-1 ring-slate-200">
                {item}
              </div>
            ))}
          </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-24 pb-32 md:px-10 md:pb-24" id="haja-contact">
        ...
        <SectionLead>방문 상담 및 입주자대표회의 검토 미팅을 위한 안내 페이지입니다.</SectionLead>

        <div className="mt-12 grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="grid gap-5">
            <InfoCard title="주요 위치">인천지방법원 · 인천지방검찰청 인근 / 학익2동 행정복지센터 인근</InfoCard>
            <InfoCard title="방문 안내" tone="beige">인천광역시 미추홀구 학익소로 29, 103호·104호<br />학익동, 석목법조빌딩</InfoCard>
            <InfoCard title="대중교통">인천지방법원·인천지방검찰청 주변 도로를 기준으로 접근 가능합니다.</InfoCard>
